...
class TokenizerBase(object):
    ...
    def encode_plus(self, passage, answer, question=None, maxlen=500):
        def create_answer_tag_ids(l1, l2):
            answer_tag_ids = []
            i = 0
            while i < len(l1):
                if l1[i: i+len(l2)] == l2:
                    answer_tag_ids += [1] * len(l2)
                    i += len(l2)
                else:
                    answer_tag_ids.append(0)
                    i += 1
            return answer_tag_ids
        passage_ids, _ = self.encode(passage)
        answer_ids, _ = self.encode(answer)
        if maxlen is None:
            # suggest in this function out to solve the problem
            assert len(passage) + len(answer) + len(question) <= maxlen

        token_ids = passage_ids + answer_ids[1:]
        segment_ids = [0] * len(token_ids)

        if question is not None:
            question_ids, _ = self.encode(question)
            token_ids = token_ids + question_ids[1:]
            segment_ids = segment_ids + [1] * len(question_ids[1:])

        answer_tag_ids = create_answer_tag_ids(token_ids, answer_ids[1:-1])
        assert len(token_ids) == len(segment_ids)
        assert len(token_ids) == len(answer_tag_ids)
        return token_ids, segment_ids, answer_tag_ids
...
if __name__ == "__main__":
    tokenizer = Tokenizer(
        os.path.join('user_data/model_data/pretrain_model', 'vocab.txt'),
        do_lower_case=True,
        # pre_tokenize=lambda s: jieba.cut(s, HMM=False)
    )

    token_ids, segment_ids, answer_tag_ids = tokenizer.encode_plus(passage='和胃的功效，能除湿利水，还有抗衰老的作用。除了最常见的茯苓饼，还可以将茯苓打磨成粉后放入粥中食用，或拌蜂蜜后抹到糕点上蒸熟食用。莲子药性平和，味道稍微有点甘甜，中医认为它能补脾胃之气，可以起到止泻的作用，另外还有养心安神的功效。芡实又名鸡头米，性平，味道甜涩，是补脾胃最好的食物之一，同时还有固津的作用它与莲子的功效相似，只不过它的收敛作用比莲子强。如果脾胃虚弱拉肚子，芡实还有助止泻',
                          answer="莲子药性平和，味道稍微有点甘甜，中医认为它能补脾胃之气，可以起到止泻的作用，另外还有养心安神的功效。芡实又名鸡头米，性平，味道甜",
                          question="莲子有什么作用？")
    print(token_ids)
    print(segment_ids)
    print(answer_tag_ids)
    tokens = []
    for token_id, answer_tag_id in zip(token_ids, answer_tag_ids):
        if answer_tag_id == 1:
